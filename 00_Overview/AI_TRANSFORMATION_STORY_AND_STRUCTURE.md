...
4. 營運主管每天早上收到 Hermes Agent briefing。Agent 已讀取 ERP 異常訂單、客服紀錄、前一天會議紀錄與 Wiki 知識，整理出三個需要追蹤的問題，並標示哪些步驟需要人工 Gate 才能繼續。
...
| 等級 | 上完課後具備的能力 | 客戶能想像的情境 |
| --- | --- | --- |
| L1 | 每位員工能在受控入口安全使用 AI | 員工有自己的 AI 工作區，不再共用帳號或任意丟資料 |
| L2 | 高手方法被整理成可複用 Skill | 新人也能沿用部門標準方法產出穩定成果 |
| L3 | AI 能串接系統並執行流程 | Gmail、Sheets、Notion、CRM、API、ERP 開始連成工作流 |
| L4 | Agent 能使用記憶、工具、Workflow 與人工 Gate | 每天自動產生 briefing、追蹤清單、證據與待批准事項 |
| L5 | 多個 Agent 能形成跨部門團隊 | 新產品、品質、醫療服務或客戶經營可以由 AI Team 協作前置分析 |

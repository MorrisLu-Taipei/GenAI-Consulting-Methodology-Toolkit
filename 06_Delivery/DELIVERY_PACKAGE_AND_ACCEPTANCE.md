...
- **L1-L3（規模軸）—— 實作並驗收的交付。** 顧問與客戶一起把 AI 導入到個人 → 部門 → 跨部門 / 全公司，每一級都有可被驗收的交付物（帳號與 Prompt Library、Skill、Workflow PoC）。這一段是可實作、可驗證、可被主管 / IT 簽核的**具體交付**。
- **L4-L5（AI 自主軸）—— 框架與顧問引導。** L4 AI 超級助理、L5 AI 團隊是公司特定、emergent 的；本方案提供**設計框架、原則與顧問引導**，由客戶自行體驗與建置，顧問從旁帶領與審視。L4-L5 是企業邁向的**頂峰願景**；D4 的 L4 PoC 聚焦在「跑通一個可追溯的閉環 + 框架移轉」，而非代為交付一套完整的內部 Agent 系統。

**術語：** 本檔的 **Stage Gate / Gate 1-5 / Gate 4A-4E ＝ 階段驗收關卡**（該階段交付物驗收通過，才進下一級）；**HITL ＝ Human-in-the-Loop（人類在迴圈內審核）**，下文「人工 Gate」即此意。完整的兩條軸說明見 [`../00_Overview/AI_TRANSFORMATION_STORY_AND_STRUCTURE.md`](../00_Overview/AI_TRANSFORMATION_STORY_AND_STRUCTURE.md) §3.0。

---

## 1.2 自己讀 vs 顧問服務：你會得到什麼

這套方法論是開源的 —— 任何人都能 clone 或下載，用 AI IDE 打開、和 AI 一起讀（見根目錄 README「把這本書讀活」）。我們很樂意你這樣用。以下誠實說明「自己讀」與「顧問服務」各自給你什麼：

**自己讀（免費）給你：**

- 完整的方法論、L1-L5 模型、八階段結構。
- 可自助填寫的成熟度問卷與自我診斷。
- 可直接複製的框架、控制表、模板、檢查清單。
- 一位會用這套方法論回答你問題、套到你情境的 AI 共讀夥伴。

**顧問服務（付費）額外給你 —— 是「螢幕上的知識」本質上給不了的：**

- **對你現場的判斷**：一個真的做過的人，看著你實際的系統、流程、資料，做框架做不了的判斷。
- **執行的手**：L1-L3 的實際建置、系統串接、跑驗收並產出 evidence。
- **客製的治理建置**：你公司實際的權限矩陣、HITL 審核、稽核、資料分級配置到位（治理無法 copy-paste）。
- **究責**：一個簽 SOW、扛風險、可被你的董事會與稽核究責的對象。
- **AI 設備**：地端 / Hybrid 部署所需的硬體與環境。
- **速度與確定性**：幾週做完、做對，由做過的人帶。

簡言之：**這本書能讓你「知道你需要什麼」；顧問服務是去「做到、做對、並且有人扛」。**
...
| L3 Workflow Blueprint | trigger、input/output schema、n8n node map、human gate、log、error handling、sample payload | L3 講師與 IT 可以直接進入 n8n 實作 |

...
> 📐 **名詞對齊(已定案)**:八階段一律用權威命名 **As-Is → Reference Model → Ideal Practice → Gap Analysis → Problem Definition → Phased Goals → To-Be Design → Implementation**(見 `../00_Overview/EIGHT_STAGE_FLOW_STORY.md` 與 `../CANONICAL_TERMS_AND_MAPS.md`)。Phase→Stage:**A=1-3 / B=4-7 / C=8**(每季回核 Stage 2)。舊報告子系統曾用 Awareness/Discovery/Stakeholder/Diagnosis/Acceptance Test 已停用並遷移。
...
│  Stage 8 Implementation：分階段實作                      │
│  + **每季 Gate C：回頭核對 Stage 2 雷達**               │
...
| --- | --- | --- | --- | --- |
| **Phase A 診斷** | 2 週 | Stage 1-3 | TOOLKIT 前半 + FRAMEWORKS 選擇器 | **中期評估報告** + Reference Model 雷達簽署 |
| **Phase B 策略** | 4 週 | Stage 4-7 | TOOLKIT 後半 + REPORT_PRODUCTION_WORKFLOW + REPORT_TEMPLATE | **完整 14 章診斷報告** + Roadmap + ROI + 治理 |
...
- **Phase A 診斷（2 週）**：用 TOOLKIT 的 Stage 1-3 工具（40 題訪談、AI/系統盤點、Reference Model 雷達），週末產出**中期評估報告**請 Sponsor 簽署。
- **Phase B 策略（4 週）**：用 TOOLKIT 的 Stage 4-7 工具 + `REPORT_PRODUCTION_WORKFLOW.md` 8 步生產流程把診斷變成簡報，填進 `CONSULTING_REPORT_TEMPLATE.md`，產出**完整 14 章報告 + 24M Roadmap + ROI**。

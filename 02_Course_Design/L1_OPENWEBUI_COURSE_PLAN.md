...
| 類型 | 檔名 / 位置 | 用途 | 狀態 |
|---|---|---|---|
| 使用者操作手冊 | §10 Deliverables（待製成 PDF） | login SOP + 模型選擇決策樹 + 檔案上傳 checklist + 資料邊界 10 案例 | ☐ 待補 PDF |
| Admin Runbook | §10 Deliverables（待製成 PDF） | 帳號 / 角色 / 群組 / 權限 / 模型 / 功能完整設定步驟 | ☐ 待補 PDF |
| Prompt Library 範本 | §6.1（5 個範例 Prompt 內嵌於講座） | Email / 會議紀錄 / 摘要 / 報告 / FAQ | ☑ 已有 |
| 資料邊界判斷題庫 | §3.3 Formative gate（10 案例） | 學員 quiz 用 | ☐ 待補 |
| Gate 1 驗收表 | §9（已有 6 項 checklist） | Summative gate 用 | ☑ 已有 |
| AI 使用規範範本 | 待補 1 頁 template | 學員可填空 | ☐ 待補 |

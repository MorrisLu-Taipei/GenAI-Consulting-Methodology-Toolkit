...
#### 6.0.1 §6.1 Foundation 完整 Lecture Map（~45 lectures × 平均 4 min = 180 min / 3 hr + 30 min buffer）
...
| --- | --- | --- | --- | --- |
| 0.1 | 歡迎來到 L2 Antigravity | TH | 3 | 講師介紹 + 為什麼 Agentic Developer 是 IT 部門的競爭力 |
| 0.2 | 你會學到什麼 | TH+SL | 2 | 4 條主 LO 預告 |
| 0.3 | **Engagement：讓 Agent 替你做一個小研究** | EN+S | 5 | Agent Manager → 派個小任務 → 看 plan / browser / artifact |

##### Section 1 — L2 定位（30 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 1.1 | L1 → L2 的跨越 | SL | 5 | Chat vs Skill 差別 |
| 1.2 | L2 兩條線：Business Skill vs Agentic Developer | SL+VS | 5 | 分流圖 |
| 1.3 | Antigravity vs 傳統 IDE | SL | 5 | 5 維對比表 |
| 1.4 | Agentic IDE 的設計哲學 | SL | 5 | Agent 自主 + Human 審查 |
| 1.5 | 三套 Codelab 對應到 L2 能力 | SL | 5 | §2 表格回顧 |
| 1.6 | 章節 recap | RC | 5 | L2 訓練分流圖 + 學習路線 |

##### Section 2 — 安裝與環境（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 2.1 | 系統需求 | SL | 4 | OS / 記憶體 / 網路 |
| 2.2 | 下載 Antigravity | S | 4 | 官網 / 平台選擇 |
| 2.3 | 安裝步驟 | S | 5 | macOS / Windows / Linux |
| 2.4 | 第一次登入 | S | 4 | Google account / 公司 SSO |
| 2.5 | Chrome / Browser 設定 | S | 4 | Antigravity Browser 介紹 |
| 2.6 | Workspace 概念 | S | 5 | 一個 workspace = 一個專案 |
| 2.7 | 建立第一個 workspace | S | 5 | New workspace 設定 |
| 2.8 | 環境健檢清單 | SL | 5 | 9 個檢查項 |
| 2.9 | 章節 recap | RC | 4 | 安裝完成驗收標準 |
| 2.10 | **Formative：環境檢查清單** | PR | 5 | 學員逐項勾選 |

##### Section 3 — Agent Manager / Editor（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 3.1 | Agent Manager 介面總覽 | S | 4 | 左 task / 中 editor / 右 artifact |
| 3.2 | 建立 Task | S | 4 | 任務描述 → plan |
| 3.3 | Implementation Plan | S | 5 | Agent 自己拆步驟 |
| 3.4 | 觀察 Agent 執行 | S | 5 | 即時看 Agent 思考 |
| 3.5 | Walkthrough Artifact | S | 5 | 證據鏈 + 重現性 |
| 3.6 | Editor 模式 | S | 4 | Agent 寫 code 即時看 |
| 3.7 | Diff review | S | 4 | 接受 / 拒絕 / 修改 |
| 3.8 | 章節 recap | RC | 4 | Agent Manager 工作流 |
| 3.9 | **Formative：完成一個小 task** | PR | 10 | 學員當場給 Agent 一個 task |

##### Section 4 — 安全與審查政策（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 4.1 | 為什麼 Agent 需要 governance | TH | 5 | 3 個失控案例 |
| 4.2 | 終端機執行政策 | S | 5 | Allow / Review / Block |
| 4.3 | Review Policy | S | 5 | 哪些動作必過 review |
| 4.4 | JavaScript 執行政策 | S | 4 | 瀏覽器 JS 風險 |
| 4.5 | Safe Mode | S | 5 | 何時啟動 |
| 4.6 | Audit Log | S | 5 | 留下所有 Agent 動作 |
| 4.7 | 治理三本柱對應企業審計 | SL | 4 | ISO / SOC2 對應 |
| 4.8 | 章節 recap | RC | 4 | 治理 checklist |
| 4.9 | **Formative：操作政策表寫一遍** | PR | 8 | 學員填自己團隊的政策 |

##### Section 5 — Browser Agent（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 5.1 | Browser Agent 介紹 | TH+S | 4 | 跟普通 selenium 差別 |
| 5.2 | 開啟網頁 + 擷取資料 | S | 5 | 抓單頁 / 抓多頁 |
| 5.3 | 互動：點擊 / 填表 | S | 5 | 模擬使用者操作 |
| 5.4 | 處理 login + cookie | S | 5 | 半自動 vs 全自動 |
| 5.5 | 結構化資料抽取 | S | 5 | JSON / CSV 輸出 |
| 5.6 | 驗證 + 截圖 | S | 4 | 證據留存 |
| 5.7 | 反爬蟲與道德邊界 | SL | 4 | robots.txt / rate limit / ToS |
| 5.8 | 章節 recap | RC | 4 | Browser Agent 使用情境清單 |
| 5.9 | **Formative：抓一個競品網站** | PR | 9 | 學員選自己感興趣的 target |

##### Section 6 — Conclusion（10 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 6.1 | 課程回顧 | TH | 3 | 4 個 LO 都達成了嗎 |
| 6.2 | L2-B Builder / GCP 路線預告 | SL | 3 | §6.2 / §6.3 下一步 |
| 6.3 | L3 銜接概念：Workflow Blueprint | TH | 2 | 為什麼還要學 L3 |
| 6.4 | 恭喜 + 下一步 | TH | 2 | 結語 |

#### 6.0.2 §6.2 Builder Lecture Map（stub）

`8 hours = ~120 lectures。Section 結構：Intro → Web research → Flask app → Iteration → Unit test/docs → L2-to-L3 bridge → I/O contract → Human gate/log → Review → Conclusion。展開時參照 §6.0.1 格式。`

#### 6.0.3 §6.3 GCP Lecture Map（stub）

`8 hours = ~120 lectures。Section 結構：Intro → GCP readiness → Architecture planning → IaC → Cloud Run service → Gemini integration → Pipeline deployment → Validation → Security extension → Conclusion。`

#### 6.0.4 §6.4 Enterprise Lab Lecture Map（stub）

`2 days = ~240 lectures。建議切成 4 個半日 mini-course，每半日依 §6.0.1 格式展開。`

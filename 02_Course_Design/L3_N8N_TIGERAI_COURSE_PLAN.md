...
## 2. 課程目標 / Learning Objectives

> 本節以 [`ONLINE_COURSE_DESIGN_METHODOLOGY.md`](ONLINE_COURSE_DESIGN_METHODOLOGY.md) §4 的 Bloom 公式撰寫：**[動詞] + [內容] + [情境]**，可驗收、可量測。

### 2.1 給 CLP 用的 4 條主 LO（學員選課前看見的承諾）

完成 L3 課程後，學員能夠：

1. **解讀** L2 產出的 Workflow Blueprint，**在 n8n 建構** Trigger / Node / Credential / Webhook / Execution 完整鏈條，**部署** 1 個可重複執行的端到端 workflow。（Apply）
2. **整合** 至少 3 個外部系統（Gmail / Sheets / API / CRM / ERP 之任 3），**使用** Gemini / AI Node 處理多模態資料（文字 / 圖片 / 語音 / 文件至少 2 種），**完成** 1 個跨系統 PoC。（Apply）
3. **設計** 人工審核 Gate（Human-in-the-Loop）、錯誤處理、通知、重試、fallback 五層治理機制，**避免** AI 自動發布高風險內容；**產出** 1 份治理檢核表。（Analyze）
4. **使用** Antigravity + TigerAI-n8n-Skill-Pack（§5.5）將自然語言意圖**生成** 可部署的 workflow JSON，**審查** 生成結果的 Trigger / Gate / 維運性，**產出** ≥ 2 個經審查的生成 workflow。（Create + Evaluate）

### 2.2 細部能力清單（章節對應）

| 主 LO | 對應細部能力 |
| --- | --- |
| LO1（基礎建構） | 讀 L2 Blueprint；建立 Trigger / Node / Credential / Webhook / Execution |
| LO2（系統整合） | 串接 Gmail / LINE / Facebook / YouTube / Sheets / Data Tables / API / CRM / ERP；用 Gemini / AI Node 處理多模態 |
| LO3（治理） | 設計 HITL Gate；錯誤處理 + 通知 + 重試 + fallback；備份 workflow 與 credential 到 GitHub |
| LO4（自動生成） | §5.5 AG + Skill Pack 安裝 / 生成 / 審查 / 部署；產出可審查的生成 workflow |

### 2.3 互動學習設計（Engagement + Formative + Summative）

> 對照 [`ONLINE_COURSE_DESIGN_METHODOLOGY.md`](ONLINE_COURSE_DESIGN_METHODOLOGY.md) §5.1 + §7 + §9.4。

**Engagement activity（開頭 10 分鐘內必須有）：**

> **打開一個既有 n8n workflow（例：講師預先準備的「Email → 客戶分類 → 寫入 CRM」）→ 學員辨識 Trigger / 找出 3 個 node / 識別 HITL Gate 在哪 / Log 寫到哪**。5-10 分鐘 hands-on，把抽象 workflow 變成「我看得懂」的具體物件。

**Formative gates（章節結尾的快速自我檢核）：**

| 章節 | Formative 檢核 | 時長 |
|---|---|---|
| §5.1 Foundation 結尾 | 學員自建 1 個 Hello World workflow（Webhook → AI Node → Sheets） | 15 分鐘 |
| §5.2 Builder 結尾 | 學員從 PoC 場景庫挑 1 題、產 workflow JSON | 30 分鐘 |
| §5.3 Advanced 結尾 | Sub-workflow + Data Tables Schema 設計題 | 20 分鐘 |
| §5.5 Skill Pack 結尾 | 同儕互審生成結果（peer review 用 §5.5.1 審查表） | 30 分鐘 |

**§5.5 Skill Pack 生成結果審查表 governance：**

- **誰審查**：L3 講師 + IT governance（如果客戶有資安團隊）
- **SLA**：學員提交生成 workflow 後，24 小時內回覆 approve / reject
- **escalation**：缺 HITL Gate 自動 reject；其他問題講師判斷可改可 reject
- **審查項目**（5 必審）：Trigger 對不對 / I/O schema 正確 / HITL Gate 存在 / Error handling 完整 / 可維運（credential 不寫死、log 有寫）

**Summative gate（全課程結尾）：**

對應 **Gate 3A-3G**（見 §9）。包含 10 個產出 → 詳見 §10 Deliverables。

### 2.4 Reference materials 清單

| 類型 | 檔名 / 位置 | 用途 | 狀態 |
|---|---|---|---|
| PoC 場景庫 | `POC_SCENARIO_SPECS.md` | 35 個實作題 | ☑ 已有 |
| n8n workflow 骨架 | `N8N_WORKFLOW_TEMPLATES.md` | 30 個 JSON 骨架 | ☑ 已有 |
| Data Tables schema 範例 | [`_deliverables/L3_DATA_TABLES_SCHEMA_EXAMPLE.md`](_deliverables/L3_DATA_TABLES_SCHEMA_EXAMPLE.md) | 5 必備欄 + 2 個完整範例 + 設計規則 | ☑ 已加 |
| Sub-workflow 設計 checklist | [`_deliverables/L3_SUBWORKFLOW_DESIGN_CHECKLIST.md`](_deliverables/L3_SUBWORKFLOW_DESIGN_CHECKLIST.md) | 8 點 checklist + 範例 + 反例 | ☑ 已加 |
| §5.5 生成結果審查表 | 上方 §2.3 已定義 5 必審項 | reviewer 用 | ☑ 已有 |
| GitHub Backup SOP | `N8N_WORKFLOW_TEMPLATES.md` 內已有 | workflow 版本管理 | ☑ 已有 |
...
### 5.0 Lecture Map（依 [`ONLINE_COURSE_DESIGN_METHODOLOGY.md`](ONLINE_COURSE_DESIGN_METHODOLOGY.md) §5.3 切細）

#### 5.0.1 §5.1 Foundation 完整 Lecture Map（~45 lectures × 平均 4 min = 180 min / 3 hr + buffer）
...
| --- | --- | --- | --- | --- |
| 0.1 | 歡迎來到 L3 n8n | TH | 3 | 講師介紹 + 為什麼 workflow 自動化是部門生產力的根基 |
| 0.2 | 你會學到什麼 | TH+SL | 2 | 4 條主 LO 預告 |
| 0.3 | **Engagement：拆一個既有 workflow** | EN+S | 5 | 講師預備 workflow → 學員辨識 Trigger / 3 node / HITL Gate |

##### Section 1 — L3 定位（30 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 1.1 | L2 Blueprint vs L3 Workflow | SL | 5 | 規格 vs 執行 |
| 1.2 | 為什麼選 n8n | SL | 5 | vs Make / Zapier / Airflow |
| 1.3 | n8n vs Antigravity 角色分工 | SL | 5 | AG = 開發；n8n = 營運 |
| 1.4 | L3 上半段 vs 下半段 | SL | 5 | 手動 vs Skill Pack 生成 |
| 1.5 | L3 完成後你會擁有什麼 | TH | 5 | 10 個 deliverable 預告 |
| 1.6 | 章節 recap | RC | 5 | L3 學習路線 |

##### Section 2 — n8n 基礎（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 2.1 | n8n 介面總覽 | S | 4 | Editor / Executions / Credentials |
| 2.2 | Trigger 節點 | S | 5 | Manual / Webhook / Cron / Event |
| 2.3 | Node 概念 | S | 5 | Input / Output / Settings |
| 2.4 | Credential 管理 | S | 5 | API Key / OAuth / 加密儲存 |
| 2.5 | Webhook 實作 | S | 5 | 接收外部資料 |
| 2.6 | Execution 觀察 | S | 5 | 成功 / 失敗 / 重跑 |
| 2.7 | 第一個 Hello World workflow | S | 6 | Webhook → Set → Sheets |
| 2.8 | 章節 recap | RC | 5 | n8n 5 大概念 |
| 2.9 | **Formative：建一個 Hello World** | PR | 5 | 學員當場建 |

##### Section 3 — Blueprint 轉換（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 3.1 | L2 Blueprint 9 欄位回顧 | SL | 5 | Trigger / Input / Process / Output / Gate / Log / Error |
| 3.2 | Trigger 轉換 | S | 5 | Blueprint Trigger → n8n Trigger node |
| 3.3 | Input Schema 對應 | S | 5 | sample payload 怎麼測 |
| 3.4 | Node map 對應 | S | 6 | Blueprint Process → n8n node 序列 |
| 3.5 | Output Schema 對應 | S | 5 | Output 寫到哪 |
| 3.6 | Sample payload 測試 | S | 5 | Curl / Postman / n8n test |
| 3.7 | 轉換表完整範例 | SL | 5 | 1 個完整 Blueprint → 1 個 workflow |
| 3.8 | 章節 recap | RC | 4 | 轉換 SOP |
| 3.9 | **Formative：轉一個 Blueprint** | PR | 5 | 學員拿 L2 Blueprint 轉 |

##### Section 4 — AI Node / Gemini（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 4.1 | AI Node 介紹 | S | 5 | Gemini / OpenAI / Anthropic / 本地模型 |
| 4.2 | 文字處理 demo | S | 5 | 分類 / 摘要 / 翻譯 |
| 4.3 | 圖片處理 demo | S | 5 | 多模態輸入 |
| 4.4 | 文件處理 demo | S | 5 | PDF / docx / pptx |
| 4.5 | Prompt 在 n8n 的最佳實踐 | SL | 5 | 跟 L1 Prompt 的差別 |
| 4.6 | Token / cost 考量 | SL | 4 | 怎麼算錢 |
| 4.7 | 多模型 fallback | S | 5 | 主用 + 備援 |
| 4.8 | 章節 recap | RC | 4 | AI Node 7 個使用情境 |
| 4.9 | **Formative：建一個 AI step** | PR | 7 | 學員加 AI Node 到自己 workflow |

##### Section 5 — Gate / Log（45 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 5.1 | 為什麼 HITL Gate 必須在 | TH | 5 | 3 個 AI 自動發布翻車案例 |
| 5.2 | Human Gate 設計 | S | 5 | 哪些條件觸發人審 |
| 5.3 | Gate 通知通道 | S | 5 | Slack / Email / LINE |
| 5.4 | Gate 審核介面 | S | 5 | n8n built-in vs 自製 |
| 5.5 | Execution Log 設計 | S | 5 | 寫哪些欄位 |
| 5.6 | 失敗測試 | S | 5 | 故意讓它失敗看怎麼處理 |
| 5.7 | 5 層治理（HITL / Error / 通知 / 重試 / fallback） | SL | 5 | 整體圖 |
| 5.8 | 章節 recap | RC | 4 | Gate + Log 設計檢核表 |
| 5.9 | **Formative：加 Gate + Log** | PR | 6 | 學員加到自己 workflow |

##### Section 6 — Conclusion（10 min）

| # | Lecture title | Type | Min | Content / Hook |
| --- | --- | --- | --- | --- |
| 6.1 | 課程回顧 | TH | 3 | 4 個 LO 達成 + Gate 3A 是否通過 |
| 6.2 | L3 Builder / Advanced 路線預告 | SL | 3 | §5.2 / §5.3 下一步 |
| 6.3 | L3 下半段 Skill Pack 預告 | TH | 2 | §5.5 自動生成 workflow |
| 6.4 | 恭喜 + 下一步 | TH | 2 | 結語 |

#### 5.0.2 §5.2 Builder Lecture Map（stub）

`8 hours = ~120 lectures。Section 結構：Intro → Trigger → Data handling → AI processing → Platform action → Human Gate → Error/retry → Demo/Review → Conclusion。`

#### 5.0.3 §5.3 Advanced Lecture Map（stub）

`8 hours = ~120 lectures。Section 結構：Intro → Sub-workflow → Data Tables → GitHub backup → Credential governance → Monitoring → L4 readiness → Conclusion。`

#### 5.0.4 §5.4 Enterprise Lab Lecture Map（stub）

`2 days = ~240 lectures，建議切 4 個半日 mini-course。`

#### 5.0.5 §5.5 AG + Skill Pack Lecture Map（stub）

`6-8 hours = ~80-100 lectures。Section 結構：Intro → Skill Pack 觀念 → 安裝 → Cookbook 模式 → Q&A 生成 → 審查 → Example finder → 部署驗收 → Conclusion。`

# 顧問方法論版簡報大綱 (30 頁) / Consulting Methodology Deck Outline (30 slides)

> Apache License 2.0 · Author: Morris Lu (盧業興) · Tiger AI 虎智科技
> Source: <https://github.com/MorrisLu-Taipei/AI-Consulting-Methodology-Toolkit>

對象 / Audience：CIO · 策略長 · 大型企業數位轉型辦公室 · 學術同行
Duration：60-75 min
Purpose：方法論深度展示，建立學術 + 實務雙重信任。

---

## Part A — 方法論基礎 (Slides 1-8)

**1. 封面**
**2. 為什麼方法論比工具重要** — Methodology > Tools
**3. 學術根源** — Prof. Michael Rosemann (QUT)；BPM Maturity Model；BPM 流程觀點
**4. 在地化動機** — 台灣 / 中文世界企業之獨特挑戰：法遵、繁中、產業結構
**5. 兩個正交框架** — L1-L5 (vertical) × 八階段顧問 (horizontal)
**6. 設計原則** — 可驗證 / 可教 / 可複製 / 可治理 (4V: Verifiable, Teachable, Replicable, Governable)
**7. 與現有框架對比** — 對照 BCG Digital Maturity / Gartner AI Maturity / MIT AI Capability：差異與互補
**8. 全圖** — 一張圖看完整套方法論

## Part B — L1-L5 成熟度模型 (Slides 9-14)

**9. L1-L5 全表**
**10. L1 Chat AI 深度**
**11. L2 Skill AI 深度**
**12. L3 Workflow AI 深度**
**13. L4 Auto Agentic AI 深度**
**14. L5 Agentic Team AI 深度** — 引用 HKUDS/ClawTeam (MIT)

## Part C — 八階段顧問結構 (Slides 15-22)

**15. 八階段全圖**
**16. Stage 1 現況掌握** — 訪談問題庫、AI Usage Inventory、Systems Inventory、As-Is Process Map
**17. Stage 2 願景對齊** — Vision Workshop、Stakeholder Alignment Matrix
**18. Stage 3 產業標竿** — Benchmark Cases (manufacturing / hospital / retail / finance / gov)
**19. Stage 4 差距分析** — Missing/Broken/Redundant、Impact×Effort、Prioritization
**20. Stage 5 高階問題定義** — Executive Problem Statement
**21. Stage 6 路徑圖** — 30/60/90 + Stage Gate
**22. Stage 7-8 解決方案 & 治理** — Skill/Workflow/Agent Map、權限、ROI、Audit

詳細工具表格：`03_Consulting_Report/CONSULTING_TOOLKIT_TEMPLATES.md`

## Part D — 評分模型與診斷 (Slides 23-25)

**23. 六大構面評分**
- 工具使用 / 知識沉澱 / 流程標準化 / 系統整合 / Agent 應用 / 治理 ROI

**24. 0-4 分量尺與成熟度判定**
**25. 主成熟度 vs 局部成熟度** — 為什麼可以局部 L4 但整體 L2

## Part E — 案例與證據 (Slides 26-28)

**26. 製造業案例摘要** — 品質改善 Agent Team
**27. 醫院案例摘要** — 病患服務改善
**28. 行銷服務業 + B2B 案例摘要**

詳細案例：`04_Scenarios/`

## Part F — 治理、ROI、永續 (Slide 29)

**29. 治理三層** — 個人 (帳號) / 部門 (Workflow Gate) / 公司 (Stage Gate + Audit)；ROI 追蹤矩陣；30/60/90 + 12 個月永續

## Slide 30 — 致謝、引用、下一步

- 恩師致謝：Prof. Michael Rosemann (QUT)
- 第三方引用：HKUDS/ClawTeam (MIT)、OpenWebUI YouTube playlist
- Apache 2.0 授權 + NOTICE 檔
- 聯絡 / 進一步研討會邀請

---

## 製作備註

- 16:9；學術級配色 (深藍 + 灰 + 一點金)
- 每章節有過場頁
- Citations 採學術腳註格式
- 完整文獻清單放附錄

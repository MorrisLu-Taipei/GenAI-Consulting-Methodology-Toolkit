...
**Git.** Git is a distributed version control system: software that records every change to a set of files as a timestamped, authored, content-addressed unit called a "commit." The chain of commits forms an immutable, replayable history of how the files reached their current state. Branches and tags name particular points in this history (a "release" is typically a named tag, e.g. `v1.0.0`). For a conventional document, "the document" refers ambiguously to whatever file the reader happens to open. For a Git-versioned artifact, "the document at commit `7da82d7`" is a precise, externally verifiable reference. This is what makes the paper's claim of "full provenance" (§1, §8) operationally meaningful rather than rhetorical: every paragraph, every table, every workflow specification is traceable to a specific moment and a specific authored decision.

**GitHub.** GitHub is the most widely used hosting service for Git repositories. For this paper's purposes, GitHub serves three distinct functions in the eBook lifecycle. First, **distribution**: anyone with internet access can clone the artifact at any prior commit, including any tagged release, without permission from or coordination with the author. Second, **collaboration surface**: Issues, pull requests, and discussions provide a public errata, contribution, and challenge channel that traditional methodology publications (book, PDF, journal article) structurally lack. Third, **DOI bridge**: GitHub's release events trigger Zenodo's webhook, which mints a persistent DOI within minutes (see §8.2 and §10.3), turning a GitHub release into a citable, archived scholarly artifact. Without this Git + GitHub + Zenodo chain, the artifact's lifecycle would still be possible but would lose much of its reproducibility, citability, and public contribution surface. References throughout this paper to "the repository," "a commit," "the v1.0.0 release," or "the workflow files" should be read in the standard Git/GitHub sense.

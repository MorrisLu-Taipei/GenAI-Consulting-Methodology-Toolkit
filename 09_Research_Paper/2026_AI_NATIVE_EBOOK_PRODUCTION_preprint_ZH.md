...
方法論工程——對顧問框架、成熟度模型、流程參考架構進行有紀律的構建、驗證與傳播——歷史上一直是緩慢的、單一作者的工藝活動。經典範例如 Capability Maturity Model [Paulk et al. 1993]、Business Process Management Maturity Model [Rosemann & de Bruin 2005]、Process Classification Framework [APQC 2022]，每一項都需要小型專家團隊耗時數年完成，以靜態文件（PDF、期刊論文）形式發布，讀者以根本上被動的方式消費：閱讀，然後嘗試應用。
...
在 DSR 範疇內，方法論發展是一類被認可的製品 [March & Smith 1995]。其下知名子類別包含**成熟度模型發展**，de Bruin 與 Rosemann [2005] 以及 Becker 等人 [2009] 為其闡明程序標準。本工作以 DSR 為主要評估視角（第 9 節），同時引入一項**二階（second-order）** 設計科學貢獻：研究中的製品不僅是一份方法論，更是**一份關於如何在 LLM 時代建構方法論的方法論**。
...
這些檔案讓全新的 LLM session 在短時間內從通用助手轉變為「方法論識讀的對話夥伴」。經驗上這是質性差異：在 repository 根目錄打開 IDE session 並自動攝取對應的環境設定檔後，第一輪即可回答相當深度的方法論問題；同一模型若無 repository context，則傾向產出較泛化的 AI 顧問建議。

換言之，reader-as-querier 並非「把文件丟給任意 chatbot」；它依賴 repository 內建的 orientation files、workflow specifications、與版本化語境，將一般 LLM session 約束成方法論識讀的查詢代理。reader-as-querier 能力因此不是任一模型本身的固有屬性，而是「模型 **加上** repository 附帶的 orientation infrastructure」所共同產生的屬性。

### 5.3 對作者—讀者關係的意涵

古典作者—讀者關係不對稱：作者廣播，讀者接收。AI 原生電子書引入第三方——讀者的 AI IDE——雙向中介此關係。讀者仍無法直接詢問作者本人，但可以詢問一個由作者公開素材塑造、以方法論為基礎的 AI。
...
傳統同儕審查的硬度隨審查者數量、領域知識與注意力分配而變化，也可能受共享盲點影響。AI 原生對抗式審查則依 workflow 設計工作量（一次性）與運算成本（變動）擴展，特別適合暴露跨檔一致性、命名漂移、依賴鏈斷裂等結構盲點。兩者不互相取代；應串聯執行。但 AI 原生層改變了方法論品質的**地板（floor）**：通過 `/devil-pair-debate` 與 `/red-team-review` 的主張，至少已通過一層可重播、可審計的機械化對抗檢查。
...
三者疊加後，方法論不再只是被發布的文件，而成為**可被追溯、討論、修正、擴充、重新發布的版本化學術製品**。我們把這個**疊加（stacking）** —— 而非任一單一層 —— 當作值得命名的貢獻。§10.3 將回到此疊加對學術出版整體的意涵。
...
此紀律處理我們認為對 AI 輔助方法論生產最深層的反對：AI 增強的作者可能傾向生成「聽起來合理的理論」，而非始終回到可檢查來源。Toolkit 的紀律——編碼於 `AGENTS.md` 並由 `/evidence-audit`（Codex）強制——讓此類生成可見且可被切除。
...
我們將 AI 原生電子書生產與三個參考點比較：McKinsey 7-Step 問題解決方法論的發展（如 [Rasiel 1999] 所載）；Rosemann 的 BPM Maturity Model 發展 [Rosemann & de Bruin 2005; de Bruin & Rosemann 2007]；以及一個代表性的 GenAI 成熟度框架（Gartner AI Maturity Model，最新公開索引版本為 2024）。
...
方法論發展的傳統成本結構一直是進入門檻。Toolkit 顯示：單一個人若有紀律地使用 AI IDE，可在數月內產出過去通常需要機構團隊與較長週期才能完成的成果。意涵不是「所有方法論工程都將變成單作者」——恰恰相反，**評估**負擔仍然巨大。但**建構**階段的進入門檻已顯著下降。
...
Toolkit 為「**IDE 即媒介**」命題提供了一個新規模的案例。Cursor、Claude Code、Antigravity、Codex 並非專為方法論工程設計，但其通用設計足以支撐此類版本化文字製品的生產。這暗示 AI IDE 設計者三個方向：
...
更重要的是，如 §8.2.1 所闡述，此 stack 應被理解為**生命週期中三個不同的層**，而非同一概念的三種包裝：Git 提供作者問責、GitHub 提供公開參與與共演化、Zenodo 提供學術固定與可引用性。這裡的出版創新不在於任一單一機制（三者皆早於本論文存在），而在於它們作為一條**整合管線（stacked composition）**被組合起來，用於方法論類製品的出版。我們主張，此組合本身值得被命名、值得在方法論出版工具集中佔有可被辯護的位置 —— 與期刊論文、學術專書並列，而非取代。

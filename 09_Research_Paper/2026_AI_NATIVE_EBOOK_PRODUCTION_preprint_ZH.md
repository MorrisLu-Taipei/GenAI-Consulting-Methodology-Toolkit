...
1. **Context window 異質。** 在本次生產過程中，較短 context 的工具較適合局部稽核與修改；全庫級綜合則主要依賴 Claude Code 的長 context（`/deep-synthesize`、`/cross-stage-trace`）。我們不對其他 IDE 的能力作普遍主張 —— 僅指出在我們特定的工具集與工作流中，長 context 引擎承擔了跨檔綜合的主要負荷。
...
這些檔案讓全新的 LLM session 在數秒內從通用助手轉變為「方法論識讀的對話夥伴」。經驗上這是質性差異：在 repository 根目錄打開 IDE session 並自動攝取對應的環境設定檔後，第一輪即可回答相當深度的方法論問題；同一模型若無 context，只會產出通用的 AI 顧問陳腔濫調。
